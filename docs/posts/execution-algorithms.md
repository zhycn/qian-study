...
执行算法就像机构投资者的"隐身衣"——大单直接砸市场等于自杀,这些算法教你"悄悄进村,打枪的不要",把大单拆成小单慢慢执行。
...
- [交易成本分析](/posts/transaction-cost-analysis) - 执行算法的目标是最小化交易成本
- [流动性](/posts/liquidity) - 流动性是执行算法设计的核心约束
- [回测](/posts/backtesting) - 执行算法的效果需要通过回测验证
