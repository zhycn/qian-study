---
title: 另类数据
description: 另类数据是量化投资的新战场——用卫星图像数停车场车辆、用社交媒体分析情绪、用信用卡数据预测业绩，在传统数据之外寻找信息差和 Alpha
category: quant
---

# 另类数据

另类数据就是当所有人都在看财报和 K 线时，聪明人已经开始数停车场里的车、刷社交媒体的情绪、追踪油轮的位置了——信息差就是 Alpha。
...
- [机器学习投资](/posts/deep-learning-quant) - 机器学习是处理另类数据的核心工具
- [因子投资](/posts/factor-investing) - 另类数据可用于构建新型因子
- [策略衰减](/posts/alpha-beta) - 另类数据因子的衰减速度通常更快
- [量化风险管理](/posts/quant-risk-management) - 另类数据引入新的合规和操作风险
- [深度学习与量化](/posts/deep-learning-quant) - 深度学习在图像和文本类另类数据处理中发挥关键作用

## 中国另类数据现状

- **数据合规**：《数据安全法》《个人信息保护法》对另类数据使用提出严格要求
- **可用数据源**：微博情绪、百度指数、电商销售数据、招聘数据等可作为另类数据
- **卫星数据**：国内商业卫星发展迅速，但金融应用仍处于早期阶段
- **信用卡数据**：中国信用卡数据获取难度大，合规风险高，不建议个人投资者使用
- **建议**：个人投资者可关注百度指数、微博情绪等公开数据源，成本较低且合规风险小
