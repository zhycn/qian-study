...
- [回测](/quant/backtesting)：组合优化策略需要通过回测验证
- [Alpha 与 Beta](/quant/alpha-beta)：组合优化决定 Alpha 策略和 Beta 暴露的配置比例
- [因子投资](/quant/factor-investing)：因子组合的权重分配是组合优化的应用
- [交易成本分析](/quant/transaction-cost-analysis)：再平衡成本是组合优化的重要约束
- [风险管理](/quant/quant-risk-management)：组合优化是风险管理的核心工具
